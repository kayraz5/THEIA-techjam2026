...
Everything shown is real output — nothing is mocked. Every demo image is asserted to be on the held-out exclusion list at build time. Run `bash scripts/demo.sh` once before recording
...
| 5 | Scoreboard from `scripts/demo.sh` — 24/24; then open `results/frozen_siglip2_giant_ship/eval/errors_fn.png` | "Degraded and clean versions score almost identically — that's the whole point. It won't always be 24 for 24, so here's what it gets wrong on the full benchmark: the same DALL-E render of a convention hall, missed clean and missed under every degradation at the same confidence. The damage isn't what breaks it — the content is. That's in our error analysis, on purpose." | 30 |
